...
class PogonaHunter(exp.Experiment):
    default_params = {
        "start_delay": 2,  # must be non-zero otherwise a block will start before the previous finishes.
        "record_video": True,
        ...
        "movementType": "low_horizontal_noise",
    ...
    def end(self):
        if exp.get_params()["record_video"]:
            schedule.once(video_system.stop_record(),interval=2*60 )
    ...
    def on_touch(self, _, payload):
        ...
        if payload["is_hit"] is True:
            schedule.once(exp.next_trial, 1)
        if (payload["is_hit"] is True) and (exp.get_params()["bugTypes"] == exp.get_params()["rewardBugs"]):
